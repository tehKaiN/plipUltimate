...
/* function pointers */
typedef uint8_t  (*pio_dev_init_t)(const uint8_t mac[6],uint8_t flags);
typedef void (*pio_dev_exit_t)(void);
typedef uint8_t  (*pio_dev_send_t)(const uint8_t *buf, uint16_t size);
typedef uint8_t  (*pio_dev_recv_t)(uint8_t *buf, uint16_t max_size, uint16_t *got_size);
typedef uint8_t  (*pio_dev_has_recv_t)(void);
typedef uint8_t  (*pio_dev_status_t)(uint8_t status_id, uint8_t *value);
typedef uint8_t  (*pio_dev_control_t)(uint8_t control_id, uint8_t value);
...
inline uint8_t pio_dev_init(pio_dev_ptr_t pd, const uint8_t mac[6], uint8_t flags)
{
...
inline uint8_t pio_dev_send(pio_dev_ptr_t pd, const uint8_t *buf, uint16_t size)
{
  pio_dev_send_t send_f = (pio_dev_send_t)pgm_read_word(&pd->send_f);
  return send_f(buf, size);
}

inline uint8_t pio_dev_recv(pio_dev_ptr_t pd, uint8_t *buf, uint16_t max_size, uint16_t *got_size)
{
...
inline uint8_t pio_dev_status(pio_dev_ptr_t pd, uint8_t status_id, uint8_t *value)
{
  pio_dev_status_t status_f = (pio_dev_status_t)pgm_read_word(&pd->status_f);
  return status_f(status_id, value);
}

inline uint8_t pio_dev_control(pio_dev_ptr_t pd, uint8_t control_id, uint8_t value)
{
